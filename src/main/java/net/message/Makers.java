...
public class Makers {
	private static Maker<SysMessage> maker = new Maker<SysMessage>() {
		...
		public SysMessage wrap(int msgId, Message msg, Map<Long, String> attachments) {
				...
				for (Entry<Long, String> longStringEntry : attachments.entrySet()) {
					KStrVPair kv = KStrVPair.newBuilder().setKey((longStringEntry).getKey()).
							setValue(null == (longStringEntry).getValue()
									? ByteString.EMPTY : ByteString.copyFromUtf8((longStringEntry).getValue())).build();
		...
		@Override
		public SysMessage wrap(int msgId, Message msg, Map<Long, String> attachments, int mapId) {
			Builder sysMessage = SysMessage.newBuilder().setVersion(ByteString.copyFromUtf8(this.version())).setMsgId(msgId);
			if (null != msg) {
				sysMessage.setInnerMsg(msg.toByteString());
			}
		...
		@Override
		public SysMessage wrap(int sequence, Integer msgId, Message msg, Map<Long, String> attachments) {
		...
		@Override
		public SysMessage wrap(int msgId, ByteString msg, Map<Long, String> attachments) {
			Builder sysMessage = SysMessage.newBuilder().setVersion(ByteString.copyFromUtf8(this.version())).
					setMsgId(msgId);
			if (null != msg) {
